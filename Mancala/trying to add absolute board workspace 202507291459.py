...
class Board:
    ...
    def get_absolute_index(self):
        '''
        Return the Absolute Index of the active position
        '''
        return self._absoluteIndex
    def get_turn(self):
        '''
    ...
    def update_absolute_index(self, newValue):
        '''
        Update the absolute index of the active position
        '''
        self._absoluteIndex = newValue 
    ...
    def distribute_dimensions_with_minimal_energy(self, input_board, board, output_board, relative_board_indices, input_board_list, absolute_input_board, absolute_index):
        '''
        Wrapper for omnibus method that contains other methods (including redistribute_first_position, reset_relative_indices, assemble_subsequent_input_board)
        These methods do bulk of the work, including redistributing marbles, resetting indices, and assembling subsequent input board
        This should probably be broken up at some point into additional methods
        ...
        while spot_on_board_is_empty == 0 and contains_repeating_configurations == 0:
            ...
            #print(absolute_index)
            #print(input_board)
            thing = absolute_index + first_position
            if (absolute_index + first_position) <= board_length:
                absolute_index = absolute_index + first_position
                #print("a")
            elif (absolute_index + pareto) <= board_length:
                absolute_index = absolute_index + pareto
                #print("b")
            else:
                absolute_index = board_length - pareto
                #print("c")
            board.update_absolute_index(absolute_index)
